...
- [ ] `harness init --profile coding-assistant <project-name>` 能正确生成项目骨架
- [ ] 生成的项目中，用户可修改 `main.py` 中的 DI 装配，替换任意组件
...
- [ ] 任意组件未注册时，DI 容器启动阶段产生可观测信息（日志或异常），不静默跳过
- [ ] 一个组件执行中抛异常时：
  - 有可追踪的日志/错误信息
  - `on_error` Hook 被触发（如果已注册）
  - 框架不崩溃（或优雅退出，不留下无提示的僵尸状态）
- [ ] MemoryBackend 文件路径不可写时，产生明确错误信息
- [ ] Tool 执行失败时（`success=False`），错误信息完整回传到 LLM 上下文（不丢失）
...
- [ ] 所有公开类有 docstring
- [ ] 每个组件有对应测试文件，且全部通过
- [ ] 所有代码符合 `05-conventions.md` 中定义的命名和结构规范
- [ ] `harness/core/` 中的代码不 import `harness/components/` 中的具体实现
- [ ] `harness/interfaces/` 中的代码不 import 任何实现模块
