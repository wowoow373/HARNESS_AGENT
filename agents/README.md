...
> 基于 Harness Runtime 构建的**领域 Agent 案例展示**。
>
> 框架整体架构、接口契约和核心组件说明请查看 [根目录 README](../README.md)。

---

## 本目录演示什么？

在 Harness Runtime 多 Agent 能力之上，本目录实现了四个 Agent 案例：

| 案例 | 路径 | 核心效果 |
|------|------|---------|
| **chat-web** | [`chat-web/`](chat-web/) | 把命令行交互替换成 WebSocket 网页聊天，支持自定义工具、emoji 渲染和会话记忆。 |
| **group-chat** | [`group-chat/`](group-chat/) | 多 Agent 与人类用户共处一个聊天室，展现差异化性格、自然抢话/插话、选择式回复与潜水机制。 |
| **customer-service** | [`customer-service/`](customer-service/) | 客服场景多意图 Agent：验证式知识问答 + 分层任务记忆 + 错误驱动边界提炼。 |
| **trajectory-analyst** | `trajectory-analyst/`（计划中） | 轨迹分析元 Agent，读取 Harness 自身的开发/运行轨迹，辅助框架自我迭代。 |

四个案例都基于同一套 Harness Runtime 接口，通过替换 `InputAdapter` / `ContextAssembler` / 添加自定义组件完成。
...
详细说明：[chat-web/README.md](chat-web/README.md)
...
详细说明：[group-chat/README.md](group-chat/README.md)
...
python customer-service/server.py
...
Runtime(console).run_from_script('customer-service/customer_service_workflow.py')
"
```
...
详细说明：[customer-service/README.md](customer-service/README.md)
...
# chat-web 测试
pytest chat-web/tests/ -v

# group-chat e2e 测试
pytest group-chat/test_e2e.py -v
